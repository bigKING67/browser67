...
function ensureBound() {
  while (endpointHealth.size > MAX_ENDPOINT_HEALTH_RECORDS) {
...
function recordTmwdTransportResult(args, transport, ok, options = {}) {
  const endpoint = options.endpoint || endpointFor(args, transport);
...
function tmwdTransportHealthSnapshot(args, transport) {
...
function preferredTmwdTransportOrder(args = {}) {
  const rows = ["ws", "link"].map((transport) => tmwdTransportHealthSnapshot(args, transport));
...
function resetTmwdTransportHealth() {
