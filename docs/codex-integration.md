...
- Run `npm run check:readiness` for the near-100 governance score. Its `ljqCtrl` row uses the same diagnostic-only Python capability probe as `check:ljqctrl`; it distinguishes default not-configured, invalid configured interpreter, importable-but-execution-gated, and execution-bridge-available states without clicking, dragging, activating windows, capturing screenshots, reading cookies, or touching clipboard.
- Run `npm run check:optional-live-proofs` when collecting near-100 optional evidence from Linux/Windows native-input hosts or approved external OAuth/SSO/MFA providers. Proof files live outside the repo by default under `~/.tmwd-browser-mcp/optional-live-proofs`, must be sanitized, and are documented in `docs/optional-live-proofs.md`. Use `npm run proof:optional-live-template` to generate safe `ok:false` starter templates instead of hand-writing proof JSON.
