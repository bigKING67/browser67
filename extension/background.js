...
function normalizeNumericTabId(raw) {
...
function requireNumericTabId(raw) {
  const tabId = normalizeNumericTabId(raw);
  if (tabId === null) throw new Error('invalid or missing numeric tabId');
  return tabId;
}
...
async function handleCDP(msg, sender) {
  const tabId = normalizeNumericTabId(msg.tabId ?? sender.tab?.id);
  if (tabId === null) return { ok: false, error: 'invalid or missing numeric tabId' };
  try {
    await chrome.debugger.attach({ tabId }, '1.3');
...
async function handleWsExec(data) {
  const tabId = normalizeNumericTabId(data.tabId);
