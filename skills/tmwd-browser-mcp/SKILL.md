...
description: >-
  Legacy alias for browser67 real-browser automation through the tmwd_browser
  MCP tool key: logged-in Chrome/Edge pages, current tabs,
  cookies/session-aware page inspection, CDP bridge commands, background tab
  actions, downloads/uploads, file chooser planning, clipboard write/paste
  wrappers, managed tab lifecycle, and native fallback planning. For page
  API/interface discovery, request initiator tracing, signature-chain tracing,
  script search, network/WS sampling, hooks, and rebuild bundles, hand off to
  the js-reverse MCP/skill backed by the same browser67 runtime.
---

# browser67 legacy browser MCP alias
...
8. Use `browser_job_ops` for long-running browser JS when synchronous tool output would be brittle. Jobs with a valid run are checkpointed outside the repo and survive MCP restart as terminal results or explicit `interrupted` recovery. `cancel` remains intent-only for already-running page JS (`abort_supported:false`), so inspect `cancel_outcome` rather than assuming execution stopped.
9. Use `browser_screenshot_ops` for real-browser PNG screenshot artifacts. Run it after `browser_tab_lifecycle.select_or_create` and `browser_wait`; use `target:"viewport"` for baseline visual QA, `target:"selector"` / `target:"clip"` for focused sections, and bounded `target:"full_page"` only with `max_pixels`. For responsive evidence, pass `viewport:{width,height,dpr,is_mobile}`; the wrapper applies `Emulation.setDeviceMetricsOverride`, verifies page viewport plus PNG dimensions against the requested viewport, and clears it after capture by default. If a responsive capture still produces a stale desktop-sized artifact, the tool returns a verification error instead of success. Pass `layout_selectors` for compact selector rect/computed-style metrics. `target:"selector"` pre-samples the target selector's layout metrics and can fall back to the measured clip when the final selector probe detaches or returns `selector_not_found`; check `selector_fallback.used` in the returned metadata. It writes artifacts outside the repo and returns path/hash/dimensions metadata, never image base64.
10. Use `browser_evidence_bundle_ops` after before/after screenshots when a design-craft L4 eval needs `screenshots.json`. It builds `design-craft.l4-screenshots.v1` from `browser_screenshot_ops` payloads, records transport/finalizer metadata when supplied, and writes only with `write:true` plus `confirm_write:true`.
11. Use `npm run runtime:cleanup:dry-run` to audit repo-external run/screenshot artifact growth. Apply retention only with the explicit `npm run runtime:cleanup -- --write` path; it keeps latest/recent active runs and deletes only complete run directories under the configured run root.
12. Use `browser_tab_ops` for list/switch/current/session selection.
13. Use `browser_file_ops` for upload inputs and native chooser planning.
14. Use `browser_download_ops` for per-run download prepare/wait/list flows.
15. Use `browser_tab_lifecycle` with `action="select_or_create"` for active work tabs. It reuses only browser67-owned managed tabs; user-opened unmanaged tabs are read-only by default and must not be navigated, mutated, or closed.
...
- Use browser67 for the user's current browser, logged-in pages, cookies, and visible tab state.
- Use browser67 wrappers for downloads/uploads/file chooser planning/clipboard write-paste/managed tab lifecycle.
- When the user is already using a page, do not take over that unmanaged tab for active work. Create or reuse a browser67-owned tab instead:
  `browser_tab_lifecycle({action:"select_or_create", url, ownership_policy:"tmwd_only", reuse_scope:"origin_path", workspace_key})`.
- Use `create_managed` only when a fresh browser67-owned tab is explicitly needed; use `fresh:true` or `reuse:false` as the escape hatch.
...
- Use `browser_job_ops.start/status/result/list/cancel` for long-running browser execution. A valid run makes the job durable and restart-recoverable, while jobs without a valid run remain non-durable. Never describe cancellation as preemptive: `abort_supported:false` means already-running page code continues, and `cancel_outcome` reports the actual effect.
...
- Use `js-reverse` for page API/interface discovery, request initiator tracing,
  ...
  uses the same browser67 runtime over the TMWD transport by default. Pages opened by `js-reverse new_page`
  are also browser67-managed; run `js-reverse finalize_task` for the same
...
- For desktop windows, native file chooser, or pure visual input, use Computer Use or browser67 native fallback.
...
- For lifecycle regressions, run `npm run check:managed-tab-live`; it opens only temporary local fixture pages and closes the browser67-owned tabs it creates.
...
- `browser_tab_lifecycle.close_unkept` must never close unmanaged existing user tabs; it only closes browser67-owned managed tabs that are not marked `keep:true`.
- `browser_tab_lifecycle.finalize_task` is the normal finalizer for Codex tasks that used browser67 managed tabs. Run it before final response/handoff for the current `workspace_key` or `task_id`, and report its `delivery_summary` plus whether tabs were closed and close-verified or intentionally kept.
- `npm run check:managed-tabs-clean` is a registry-only hygiene gate for missed finalizers; run it after lifecycle changes or when diagnosing tab buildup. It groups leaks by cleanup scope, reports duplicate URL groups and old unkept records, and prints scoped finalize suggestions. `npm run verify` snapshots a temporary managed-tab baseline first and fails only on newly leaked unkept records, while the standalone gate remains globally strict.
