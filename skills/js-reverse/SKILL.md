...
- 长任务 readiness / 证据化作业：用 `browser_transport_health` 诊断 browser67 transport（`ws`/`link`），用 `browser_wait` 替代固定 sleep，用 `browser_run_ops` 写 repo 外 run artifact，用 `browser_job_ops` 承载长 JS 任务（当前 `durable:false`，`cancel` 不抢占页面 JS）
